...
var handlers = [256]func(c *threadedCodeCompiler) func(i *Interpreter){
	...
	instr.CALL: func(c *threadedCodeCompiler) func(i *Interpreter) {
		...
		return func(i *Interpreter) {
			...
			case *types.Function:
				...
				if i.sp+len(fn.Locals)-1 >= len(i.stack) {
					panic(ErrStackOverflow)
				}
				for idx := 0; idx < len(fn.Locals); idx++ {
	...
	instr.RETURN: func(c *threadedCodeCompiler) func(i *Interpreter) {
		...
		return func(i *Interpreter) {
			...
			if i.sp < len(fn.Typ.Returns) {
				panic(ErrStackUnderflow)
			}
			copy(i.stack[f.bp:f.bp+len(fn.Typ.Returns)], i.stack[i.sp-len(fn.Typ.Returns):i.sp])
			i.sp = f.bp + len(fn.Typ.Returns)
			i.release(f.addr)
			f.code = nil
			i.fp--
		}
				...
				case instr.CALL:
					...
					case *types.Function:
						return func(i *Interpreter) {
							...
							if i.sp+len(fn.Locals) >= len(i.stack) {
								panic(ErrStackOverflow)
							}
							for idx := 0; idx < len(fn.Locals); idx++ {
								i.stack[i.sp+idx] = 0
