...
func (l arm64Lowerer) lower(ctx *lowering, plan plan) bool {
	...
	for n := 0; n < len(ctx.work); n++ {
		sort.SliceStable(ctx.work[n:], func(i, j int) bool {
			return ctx.work[n+i].hits > ctx.work[n+j].hits
		})
		work := ctx.work[n]
...
func (l arm64Lowerer) steps(ctx *lowering, ops []step) (bool, bool) {
	for idx := 0; idx < len(ops); idx++ {
		...
		case instr.I32_CONST,
			...
			instr.F64_GE,
			instr.ARRAY_GET:
			handled, emitted := l.emitStep(ctx, op)
			ok = handled && emitted
...
func (l arm64Lowerer) target(ctx *lowering, source, consumer step) bool {
	...
	if callee < 0 || callee >= len(ctx.funcs) || callee != consumer.callee || ctx.funcs[callee] == nil {
		return false
	}
...
func (l arm64Lowerer) emitStep(ctx *lowering, op step) (bool, bool) {
	ok := false
	switch op.op {
	case instr.I32_CONST:
...
func (l arm64Lowerer) enterBlock(ctx *lowering, state state) {
