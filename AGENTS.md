...
- A native loop callable may return `trapNone` (a folded depth-0 `RETURN` leg emits a bare `ret()`, a folded completed leg emits `complete()`); `interp.loop()` must then tear down the frame like `call()` for a function loop or mark the module frame exhausted like `start()` — silently ignoring `trapNone` livelocks dispatch.
- Only a partial leg whose cut lands on the plan's own header (same function, depth 0) may fold into the native loop back-edge; loop-kind legs are never split (anchored at the header they are the root itself, anchored elsewhere they are a different loop), and `label()` must share a continuation's label when block, tail, and canonical snapshot are identical — its ledger keeps consumed work items so leg cycles converge instead of exhausting `continuationLimit`.
