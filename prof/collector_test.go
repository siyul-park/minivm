...
func TestCollector_AddMetric(t *testing.T) {
	collector := NewCollector()
	collector.AddMetric("custom", 2, Label{Key: "mode", Value: "jit"})
	collector.AddMetric("custom", 3, Label{Key: "mode", Value: "jit"})
	require.Equal(t, float64(5), collector.Value("custom", Label{Key: "mode", Value: "jit"}))
}
...
func TestCollector_Metrics(t *testing.T) {
	t.Run("execution metrics", func(t *testing.T) {
		...
		require.Contains(t, metrics, Metric{Name: "vm_samples_total", Value: 1})
		require.Contains(t, metrics, Metric{Name: "custom", Value: 2})
	})

	t.Run("jit metrics", func(t *testing.T) {
		c := NewCollector()
		c.RecordCapture(1, 2, CaptureOutcomePartial, CaptureReasonOpLimit)
		c.RecordCompile(1, 2, TriggerHot, FrontendTrace, CompileOutcomeEmitted, CompileReasonNone)
		c.RegisterYield(1, 2, EntryStart, FrontendTrace).Inc()
