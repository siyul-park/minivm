...
func TestGuardPass_Run(t *testing.T) {
	t.Run("collapses a repeated shape guard over the same operand", func(t *testing.T) {
		...
		b.Add(entry, ssa.Operation{Op: ssa.OpGuardShape, Shape: ssa.Shape{Tag: 5}, Args: []ssa.Value{array}, State: state, Results: []ssa.Value{first}})
		second := b.Value(ssa.TypeRef)
		b.Add(entry, ssa.Operation{Op: ssa.OpGuardShape, Shape: ssa.Shape{Tag: 5}, Args: []ssa.Value{array}, State: state, Results: []ssa.Value{second}})
	...
	t.Run("does not collapse guards admitting different shapes", func(t *testing.T) {
		...
		b.Add(entry, ssa.Operation{Op: ssa.OpGuardShape, Shape: ssa.Shape{Tag: 5}, Args: []ssa.Value{array}, State: state, Results: []ssa.Value{first}})
		second := b.Value(ssa.TypeRef)
		b.Add(entry, ssa.Operation{Op: ssa.OpGuardShape, Shape: ssa.Shape{Tag: 6}, Args: []ssa.Value{array}, State: state, Results: []ssa.Value{second}})
	...
	t.Run("preserves a deopt frame's own values while eliminating a redundant guard sharing its state", func(t *testing.T) {
		...
		first := b.Value(ssa.TypeRef)
		b.Add(entry, ssa.Operation{Op: ssa.OpGuardShape, Shape: ssa.Shape{Tag: 9}, Args: []ssa.Value{array}, State: state, Results: []ssa.Value{first}})
		second := b.Value(ssa.TypeRef)
		b.Add(entry, ssa.Operation{Op: ssa.OpGuardShape, Shape: ssa.Shape{Tag: 9}, Args: []ssa.Value{array}, State: state, Results: []ssa.Value{second}})
		b.Term(entry, ssa.Terminator{Op: ssa.OpReturn, Args: []ssa.Value{second}})
		fn := b.Build()
		require.NoError(t, ssa.Verify(fn))
