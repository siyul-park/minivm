...
func TestCSEPass_Run(t *testing.T) {
	t.Run("collapses a repeated pure computation within one block", func(t *testing.T) {
		...
		b.Add(entry, ssa.Operation{Op: ssa.OpExec, Code: instr.I32_ADD, Args: []ssa.Value{x, y}, State: deoptState(b, entry, instr.I32_ADD), Results: []ssa.Value{first}})
		b.Add(entry, ssa.Operation{Op: ssa.OpExec, Code: instr.I32_ADD, Args: []ssa.Value{x, y}, State: deoptState(b, entry, instr.I32_ADD), Results: []ssa.Value{second}})
	...
	t.Run("collapses a computation a dominated block repeats", func(t *testing.T) {
		...
		b.Add(next, ssa.Operation{Op: ssa.OpExec, Code: instr.I32_ADD, Args: []ssa.Value{x, y}, State: deoptState(b, entry, instr.I32_ADD), Results: []ssa.Value{second}})
	...
	t.Run("does not collapse a computation two sibling arms repeat independently", func(t *testing.T) {
		...
		b.Add(left, ssa.Operation{Op: ssa.OpExec, Code: instr.I32_ADD, Args: []ssa.Value{x, y}, State: deoptState(b, entry, instr.I32_ADD), Results: []ssa.Value{l}})
		b.Term(left, ssa.Terminator{Op: ssa.OpJump, Edges: []ssa.Edge{{Block: join, Args: []ssa.Value{l}}}})
		r := b.Value(ssa.TypeI32)
		b.Add(right, ssa.Operation{Op: ssa.OpExec, Code: instr.I32_ADD, Args: []ssa.Value{x, y}, State: deoptState(b, entry, instr.I32_ADD), Results: []ssa.Value{r}})
