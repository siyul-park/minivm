...
//   bits[7:0]   = nParams
//   bits[15:8]  = nReturns
//   bits[23:16] = nScratch    (≤ 6; scratch registers X10–X15 in order)
...
//   argv[nScratch+1..]: params in / returns out (max(nParams,nReturns) slots)
//
...
TEXT ·invoke(SB), NOSPLIT, $0-16
    ...
    UBFX $16, R9, $8, R10       // R10 = nScratch
    ADD $8, R8, R14              // R14 = argv+8
    LSL $3, R10, R10             // R10 = nScratch*8
...
load:
    // Load scratch inputs (X10–X15) from argv[1..nScratch].
    MOVD argv+8(FP), R8         // R8 = argv
    MOVD 0(R8), R9              // R9 = header
    UBFX $16, R9, $8, R9        // R9 = nScratch
...
call:
    ...
    // Save scratch outputs (X10–X15) to argv[1..nScratch].
    // X8 and X9 are not scratch inputs/outputs, so R8 and R9 are free as temporaries.
    MOVD argv+8(FP), R8         // R8 = argv
    MOVD 0(R8), R9              // R9 = header
    UBFX $16, R9, $8, R9       // R9 = nScratch
    ADD $8, R8, R8              // R8 = argv+8 (scratch slots base)
