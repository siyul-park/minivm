...
|---|---|---|---|---|---|
| native Go | 18,912,368 | 0 | 0 | 1× | 컴파일 |
| wazero | 42,690,476 | 16 | 2 | 2.3× | WASM → 네이티브 JIT |
| **minivm (JIT)** | **71,776,495** | **1,727** | **21** | **3.8×** | **스레디드 인터프리터 + ARM64 JIT** |
| minivm (interp) | 750,025,292 | 139 | 0 | 40× | 스레디드 인터프리터 |
| tengo | 1,142,455,492 | 312,798,644 | 39,088,178 | 60× | 바이트코드 VM |
| gopher-lua | 1,422,659,979 | 971,072 | 3,793 | 75× | 레지스터 VM |
| goja | 2,032,321,458 | 379,440 | 46,376 | 107× | 바이트코드 VM |

JIT는 이 워크로드에서 **10× 효과**를 냅니다(호출당 750 ms → 72 ms). 순수 인터프리터 중에서는 minivm (interp)이 선두이며 사실상 할당이 없습니다: **tengo보다 1.5×, gopher-lua보다 1.9×, goja보다 2.7× 빠르며**, tengo가 312 MB를 할당하는 지점에서 0 allocs/op입니다. JIT를 켜면 minivm은 네이티브 코드에 도달하는 유일한 런타임인 wazero와 어깨를 나란히 하며, 스크립트 VM 대비 **16–28× 앞섭니다**.

minivm의 JIT는 숫자 세그먼트만이 아니라 함수 전체를 컴파일합니다. fib의 재귀 `const.get; call`은 호출 대상의 프레임 진입점으로 향하는 네이티브 branch-and-link로 융합되므로, 재귀 전체가 네이티브 코드로 실행됩니다. 그럼에도 wazero에 1.7× 뒤지는 이유는 wazero가 생략하는 부가 작업 때문입니다. minivm은 값을 NaN-boxed 표현으로 유지하고 모든 호출에서 프레임 예산 검사와 deopt 저널 기록을 수행하는 반면, wazero는 폴백 경로 없이 언박싱된 네이티브 코드로 AOT 컴파일합니다.
