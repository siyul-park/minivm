...
재귀 `fib(35)` — 호출당 엔드투엔드, linux/amd64:
...
**JIT 없는 인터프리터 중에서** minivm은 tengo(~1.6×), gopher-lua(~2.4×), goja(~3.2×)보다 빠름. wazero가 앞서는 이유는 로드 시점에 WebAssembly를 x86-64 네이티브 코드로 JIT 컴파일하기 때문 — minivm은 ARM64에서 핫 세그먼트가 네이티브로 승격되면 이 차이가 줄어듦.
