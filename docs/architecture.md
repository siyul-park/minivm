...
`jitCompiler` (`jit.go`, `jit_arm64.go`): ARM64 JIT kept private inside `interp`. Its opcode walk intentionally mirrors `threadedCompiler`: decode opcode, lower through a switch, advance IP unless the opcode terminates the segment. Native code uses a scratch-only `argv []uint64` call boundary, loads VM stack inputs directly, keeps operands in registers, and materializes `i.stack`, `sp`, and `ip` only on exits. Complete numeric direct-call SCCs lower `CONST_GET function; CALL` to native `BL` without touching `frame.code` or `frame.upvals`; incomplete functions still compile supported partial segments around threaded calls. JIT does not recompile or tier-up.
...
4. `Link`: writes code into the executable buffer with `buffer.Unseal()` → `buffer.Append(code)` → `buffer.Seal()`, resolves relocations, and creates primary/internal `Callable`s.
