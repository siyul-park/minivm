...
`threadedCompiler` (`threaded.go`): `[256]func` table populated in `init()`. Each compile-time entry reads operands, advances `c.ip`, and returns a runtime closure that captures constants and advances `f.ip` by instruction width. An entry may fuse with what follows into a single superinstruction closure (`interp/fuse.go`): it *probes* ahead by temporarily advancing `c.ip` into the next instruction(s), decides whether to fuse, and — whether or not it fuses — *restores* `c.ip` to right after its own width before returning. This restore is load-bearing: `Compile`'s outer loop only ever visits an `ip` some entry's own advance landed on, so restoring `c.ip` is what makes the loop independently compile a standalone, correct closure for every instruction-start byte a fusion "skips" at runtime — including ones a `BR`/`BR_IF`/`BR_TABLE` target may land on directly. `fuseLocalConst`/`fuseLocalLocal` fold `LOCAL_GET idx; <kind>.CONST c; <binop>` and `LOCAL_GET a; LOCAL_GET b; <binop>` into one dispatch per numeric kind, reading frame slots directly instead of round-tripping through the stack; I64 retains the local's boxed value before `unboxI64` (which releases) so the local slot's own reference stays balanced. Comparison opcodes (`I32_EQ`/`I64_LT_S`/`F32_GE`/etc., plus `I32_EQZ`/`I64_EQZ` and `<kind>_CONST`) peek one instruction ahead via `peekBrIf` for an immediately following `BR_IF`, and if present fuse into a closure that computes the Go `bool` directly and calls the shared `Interpreter.branchIf` — skipping the boxed-`bool` push/pop a plain `<cmp>; BR_IF` pair would otherwise do. `peekBrIf`/`branchIf` are reused by `fuseLocalI32Const`/`fuseLocalI64Const`/`fuseLocalF32Const`/`fuseLocalF64Const`'s own comparison cases too, so a `LOCAL_GET a; CONST c; cmp; BR_IF` loop-header shape collapses to a single dispatch when all four line up.
