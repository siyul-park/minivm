...
A specialized-type miss on either fusion (e.g. a generic `*types.Array` or `*HostObject` heap value under a container declared with a concrete element or struct type) falls back to `(*Interpreter).arrayElem` or `(*Interpreter).structField` in `interp/interp.go` instead of trapping a case the unfused handler accepts, regardless of which of the three container sources it came from. The standalone `ARRAY_GET`/`STRUCT_GET` handler calls the same method unconditionally, so the generic aggregate-read dispatch is one hand-written runtime method shared by every fused and unfused caller instead of a switch duplicated into generated code.
