...
A specialized-type miss on either fusion (e.g. a generic `*types.Array` or `*HostObject` heap value under a container declared with a concrete element or struct type) falls back to the same unfused arm the standalone handler runs (`arrayGetElem`, `structGetHostObject`) instead of trapping a case the unfused handler accepts, regardless of which of the three container sources it came from.
