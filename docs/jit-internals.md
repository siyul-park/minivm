...
A bridge cycle re-enters through a fresh external `Call`, which never runs the loop-carry prologue (see `arm64Lowerer.dispatch` above): a carried register would be uninitialized garbage on such a resume. A bridge anywhere in a static plan therefore keeps every local slot-backed instead of loop-carried, even in a loop the bridge itself never touches; narrowing this to "a bridge inside the loop body" is unimplemented follow-up work, tracked because it would need the carry-load prologue to run on every re-entry path, not just the callable's own head.

`arrayKind` (`interp/jit_plan.go`) resolves an `ARRAY_GET`/`ARRAY_DELETE` element kind only from a known constant container's concrete heap itab, matching `arrayGetKnown`'s native lowering. A declared-array-type fallback (mirroring `structFieldKind`'s declared-struct-type resolution, which STRUCT_GET already relies on) was tried and reverted: it let `ARRAY_GET`'s general (non-constant) lowering path run for the first time, and that combination — a native `CALL` and a resolved-kind `ARRAY_GET` in the same plan — corrupted native execution state on the next native call boundary (`runtime.mallocgc` SIGSEGV) instead of cleanly guarding and falling back the way `structFieldKind`'s equivalent case does. Root-causing that interaction in `arm64Lowerer.arrayGet`/`arrayLen` is unimplemented follow-up work; until then, `ARRAY_GET`, `ARRAY_LEN`, and `ARRAY_DELETE` stay resolvable only for a known constant container.
