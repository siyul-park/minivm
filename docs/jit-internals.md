...
A recursive `CALL` from a non-entry loop trace is a hard trace boundary. The recorder cannot model the recursive callee by `skipCall`: that path supplies zero returns and does not reproduce the callee's heap mutations. The loop prefix therefore cuts at the recursive call, lets threaded execution perform the real call, and records the post-call continuation separately against the actual frame and heap state. Function-entry traces retain native self-call lowering because that call is part of their native frame contract.
