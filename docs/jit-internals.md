...
- **No hoist, no carry.** `Plan.Hoist` and `Plan.Carried` have no SSA form, deliberately. Both are backend derivations: the IR already states the per-access `OpGuardShape` that `hoistable` counted, and hoisting one is loop-invariant code motion over explicit guards, which an optimizer pass with `internal/graph` dominance does more generally than one container per plan — without `hoistable`'s ref-array and `MaxHoistSlot` restrictions, which exist only because a cached slice header is an untracked backend representation. `Carried` is likewise which locals an allocator keeps in registers across a back-edge.
