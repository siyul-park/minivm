...
`frontend.Body(m, addr, fn)` is that same translation over a whole function against a `Module` alone, for the ahead-of-time optimizer (`transform.SSAPass`). It skips exactly the three rules that are the JIT's and not the translation's: the anchor must be the function entry rather than a loop header, an already-installed entry is still translated, and module code holding a `CALL` is accepted because nothing here has to implement the framed native-call ABI. Address zero still means module code, which ends on `OpComplete` rather than a return.
