...
For a function entry, probing starts with a 32-reach warm window that is not judged. It then alternates native and shadow windows, starting at 32 reaches and growing to at most 256 when the confidence interval remains wider than 5%. Only the first and last reach of each timed window take `time.Now()`, so no per-reach clock call is added. During a shadow window, the wrapper invokes its saved `resumeShadowed` handler instead of native code. The function's `i.natives` fast-path slot is atomically cleared for the whole shadow window and restored after the final shadow reach; native-to-native calls therefore cannot bypass the probe. Each native/shadow pair contributes a relative throughput delta to an online mean and variance. After three pairs, the probe retires when the 95% confidence interval is entirely above a 1% improvement, keeps native execution when it is entirely below a 1% regression, grows the window when uncertainty is still high, and gives up conservatively after seven pairs. A decided anchor never probes again.
