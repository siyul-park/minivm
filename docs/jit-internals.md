...
A call whose callee is the function being compiled takes the native self-call path (`BL` to `ctx.head`) from either call form: the guarded function-value form through `call`, and the fused constant-marker form through `directCall`. The `BLR` slot path cannot serve it, because the callable is still being emitted and has published no slot. `selfCall` owns that branch's preconditions rather than either call site, adding to the live-frame requirement below a rejection of any target with captures, whose closure body would need them rebound. Lowering the fused form is what lets the static frontend plan a recursive function at all; while it was rejected, such a function had no whole-function plan and fell back to a recorded trace, whose coverage varied with how much of the recursion the recording happened to reach. Recursive calls encountered inside loop traces are still cut at the call so the continuation is compiled from the real post-call state.

`selfCall` lowers that recursion as a `BL` to `ctx.head`, which re-enters the plan's entry prologue, so it requires the live frame to be the plan's own: `ctx.kind == entryFunction` and exactly one frame. Neither holds automatically. The trace frontend records any `CALL` whose callee ref matches a function-entry anchor as an ordinary self `CALL`, including one nested inside an already-inlined callee's body, so `A` inlining `B` and `B` calling back into `A` reaches `selfCall` with two frames live; the `BL` then lays `A`'s parameter prologue over `B`'s activation. With differing frame shapes that corrupts native state badly enough to fault inside the Go runtime. `TestARM64_SelfCallFromInlinedFrame` pins it.
...
That terminality is load-bearing, not leftover conservatism, and both attempts to lift it were reverted. The store itself is fully emitted and guarded before the exit, so lifting the rule looks safe in isolation — but letting either store continue natively corrupts refcounts, driving them negative against a threaded twin, once the continuation is itself nested inside a native self-recursive call. Both container kinds fail identically, from recursion depth two upward; depths zero and one pass, so a shallow test does not see it. The defect is in that interaction rather than in the store's own guards, and it is not yet root-caused. `TestARM64_RefContainerStoreRemainsTerminal` pins the rule for both kinds. Note the cost: a terminal store that is not its block's last step makes `steps` report `ok=false`, which rejects the whole plan rather than truncating the block, so a ref store in the middle of a loop body keeps its entire function threaded.
