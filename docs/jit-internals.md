...
Refute and retire (S2-P6c): every deoptimization counts against the address that deoptimized; once that count reaches `native`'s `refute` threshold, the interpreter retires the address's code (`Store.Retire`) once it is back outside native code (after `Store.Leave`) and resets its call, entry, and deopt counts, so it tiers up again from a fresh Baseline compile if it is still called. A permanent compile failure is unaffected by retirement, since it does not depend on runtime behavior.
