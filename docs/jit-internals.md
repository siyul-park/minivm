...
Linear spill state is unsafe across control flow, and mutation blocks can combine paths around state materialization. Three layers enforce safety:

- `internal/asm/rewriter.go`'s `victim` never chooses a value whose live range spans a bound label. The allocator places a spill's store where register pressure was observed, which can sit on only one path into that label, while the reload after it runs on every path; the value then reads a slot the taken path never wrote. `TestAssembler_Build/spills a value live across a forward branch` builds exactly that shape, poisons the slot first so the corruption is a fixed constant rather than arbitrary garbage, and drives the diamond both ways.
- `internal/asm/rewriter.go` rejects spilling for code containing an intra-code backward branch. The live-range rule does not subsume this: a back edge re-enters above its label, so a range that appears to end before the branch is still live on the next iteration without spanning any label.
