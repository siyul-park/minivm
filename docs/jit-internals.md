...
For a function entry, probing starts with a 64-reach warm window that is not judged. It then alternates five 64-reach native windows with five shadow windows. Only the first and last reach of each timed window take `time.Now()`, so no per-reach clock call is added. During a shadow window, the wrapper invokes its saved `resumeShadowed` handler instead of native code. The function's `i.natives` fast-path slot is atomically cleared for the whole shadow window and restored after the final shadow reach; native-to-native calls therefore cannot bypass the probe. A round wins only when shadow time is more than 1% lower than native time, and a majority of the five rounds retires the anchor. A non-winning majority marks it decided and probing never runs again.
