...
func (m machine) dispatch(ctx *lowering, vCtrl asm.VReg) {
	var resumable []int
...
func (m machine) emitExits(ctx *lowering) bool {
	...
	for _, exit := range ctx.sideExits {
		...
		m.trapFlushed(ctx, journal.TrapFallback, exit.resume, exit.id)
	}
	return true
}
...
func (m machine) guardDivisor(ctx *lowering, divisor value, reg asm.VReg, observed uint64, ip int) bool {
	guarded := false
	if !divisor.known && observed != 0 {
		if !m.guardRaw(ctx, reg, observed, ip) {
			return false
		}
		guarded = true
	}
	...
	fail, ok := m.sideExit(ctx, ctx.values, ip, prof.ExitGuardValue, ctx.opcode(ip))
	if !ok {
		return false
	}
...
func (m machine) boxableI64(ctx *lowering, raw asm.VReg, ip int) bool {
	fail, ok := m.sideExit(ctx, ctx.values, ip, prof.ExitGuardValue, ctx.opcode(ip))
	if !ok {
		return false
	}
	m.guardBoxable(ctx, raw, fail)
	return true
}
...
func (m machine) guardRaw(ctx *lowering, got asm.VReg, val uint64, ip int) bool {
	fail, ok := m.sideExit(ctx, ctx.values, ip, prof.ExitGuardValue, ctx.opcode(ip))
	if !ok {
		return false
	}
...
func (m machine) guardI64(ctx *lowering, v asm.VReg, ip int) bool {
	fail, ok := m.sideExit(ctx, ctx.values, ip, prof.ExitGuardKind, ctx.opcode(ip))
	if !ok {
		return false
	}
...
// i64 value.
func (m machine) sign64(ctx *lowering, v asm.VReg) asm.VReg {
...
func (m machine) exit(ctx *lowering, resume int, reason prof.ExitReason, opcode int) bool {
	return m.trap(ctx, journal.TrapFallback, resume, reason, opcode)
}
...
func (m machine) bridge(ctx *lowering, ip int) bool {
	return m.trap(ctx, journal.TrapBridge, ip, prof.ExitNone, prof.OpcodeNone)
}
...
func (m machine) trap(ctx *lowering, kind journal.Trap, resume int, reason prof.ExitReason, opcode int) bool {
	...
	m.trapFlushed(ctx, kind, resume, id)
	return true
}

func (m machine) trapFlushed(ctx *lowering, kind journal.Trap, resume, exitID int) {
	a := ctx.assembler
	...
	m.report(ctx, vCtrl, kind, resume)
	a.Emit(
		arm64.RET(),
	)
}
...
func (m machine) unwind(ctx *lowering, vCtrl asm.VReg, resume int) {
	for k := len(ctx.frames) - 1; k >= 0; k-- {
		...
		m.save(ctx, vCtrl, f, ip)
	}
}

func (m machine) save(ctx *lowering, vCtrl asm.VReg, f *activation, ip int) {
	a := ctx.assembler
...
func (m machine) report(ctx *lowering, vCtrl asm.VReg, trap journal.Trap, nextIP int) {
	a := ctx.assembler
...
func (m machine) sideExit(ctx *lowering, pre []value, resume int, reason prof.ExitReason, opcode int) (asm.Label, bool) {
