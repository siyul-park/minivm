...
func renderDocs(rules []rule) []byte {
	...
	for _, rule := range rules {
		source := operationName(rule.pattern[0])
		count := counts[source]
		count.threaded++
		if rule.arm64 {
			count.arm64++
	...
	for _, source := range sources {
		count := counts[source]
		fmt.Fprintf(&matrix, "| `%s` | %d | %d |\n", source, count.threaded, count.arm64)
	}
	fmt.Fprintf(&matrix, "| Total | %d | %d |\n", len(rules), arm64Total)
...
func supportsARM64(pattern pattern) bool {
	...
	if consumer == instr.CALL || consumer == instr.RETURN_CALL {
		return len(pattern) == 2 && source == instr.CONST_GET
	}
